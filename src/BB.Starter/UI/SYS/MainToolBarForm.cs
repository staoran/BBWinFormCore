...
public partial class MainToolBarForm : ToolbarForm
{
    #region 属性变量
    ...
    //用来第一次创建动态菜单
    private ToolBarMenuHelper _menuHelper;
    ...
    public MainToolBarForm()
    {
        ...
        // 初始化菜单帮助类
        _menuHelper = new ToolBarMenuHelper(this, ref toolbarFormControl1);
    ...
    private void MainToolBarForm_MaximizedBoundsChanged(object? sender, EventArgs e)
    {
        Hide();
    }
    ...
    /// 缩小到托盘中，不退出
    /// </summary>
    private void MainToolBarForm_FormClosing(object sender, FormClosingEventArgs e)
    {
    ...
    /// <param name="sender"></param>
    /// <param name="e"></param>
    private void MainToolBarForm_Move(object? sender, EventArgs e)
    {
        //最小化到托盘的时候显示图标提示信息
    ...
    /// <param name="sender"></param>
    /// <param name="e"></param>
    private async void MainToolBarForm_Load(object? sender, EventArgs e)
    {
        try
        {
            #region 加载皮肤选择器
