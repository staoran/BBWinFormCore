...
/// </summary>
public partial class RowMergeGridView : DataGridView
{
    #region 构造函数
    public RowMergeGridView()
    {
        InitializeComponent();
    }
    #endregion
            ...
            else
            {
                ...
                if (e.RowIndex == -1)
                {
                    if (_spanRows.ContainsKey(e.ColumnIndex)) //被合并的列
                    {
                        ...
                        //画上半部分底色
                        g.FillRectangle(new SolidBrush(_mergecolumnheaderbackcolor), left, top,
                            right - left, (bottom - top) / 2);

                        //画中线
                        g.DrawLine(new Pen(GridColor), left, (top + bottom) / 2,
                            right, (top + bottom) / 2);
    ...
    private void DrawCell(DataGridViewCellPaintingEventArgs e)
    {
        ...
        if (MergeColumnNames.Contains(Columns[e.ColumnIndex].Name) && e.RowIndex != -1)
        {
            ...
            string curSelected = CurrentRow.Cells[e.ColumnIndex].Value == null ? "" : CurrentRow.Cells[e.ColumnIndex].Value.ToString().Trim();
    ...
    {
        get => _mergecolumnheaderbackcolor;
        set => _mergecolumnheaderbackcolor = value;
    }
