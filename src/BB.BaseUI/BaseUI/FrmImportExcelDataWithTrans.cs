...
/// 通用Excel数据导入操作
/// </summary>
public partial class FrmImportExcelDataWithTrans : BaseForm
{
    private AppConfig _config = new();
    private DataSet _myDs = new();
    ...
    /// 默认构造函数
    /// </summary>
    public FrmImportExcelDataWithTrans()
    {
        InitializeComponent();
            ...
            try
            {
                foreach (DataRow dr in dt.Rows)
                {
                    if (OnDataSave != null)
                    {
                        ...
                        try
                        {
                            // todo 改成批量操作，整个表传出去或者用泛型（增加处理委托和批量保存）
